...
# 
# In this analysis data for 2017 is considered, with demand and dispatch time series re-sampled to 30min intervals, corresponding to the length of a trading interval within Australia's National Electricity Market (NEM). For each week in 2017 a reduced set of 10 operating scenarios with associated durations is created. This reduces the number of scenarios from:
# 
# - 365 days/year $\times$ 48 trading intervals/day = 17520 trading intervals/year 
# 
# to 
# 
...
def create_scenarios(df, k=1, max_iterations=100, stopping_tolerance=100):
    ...
    def compute_distance(col):
        ...
        closest_centroid_ID = pd.Series(data={'closest_centroid': closest_centroid, 
    ...
    for i in range(max_iterations):
        ...
        print('Iteration number: {0}, total absolute distance: {1}'.format(i+1, total_absolute_distance))
        
        # Raise warning if loop terminates before stopping condition met
        if i == (max_iterations - 1):
            raise(Warning('Max iteration limit exceeded before stopping tolerance satisfied.'))
...
# Split time index into chunks. Each chunk corresponds to one week.

# In[7]:


# Number of trading intervals per week = chunk size
# (7 days/week * 48 trading intervals/day = 336 trading intervals/week)
chunk_size = 336

# Map between week number and the timestamps of trading intervals for that week
week_trading_intervals_map = dict()

# For each week in the year
for week in range(1, 54):
    
    # Find timestamps corresponding to each week
    timestamps = df_o.columns[(week-1)*chunk_size:week*chunk_size]
    
    # If a full 7 days of time stamps
    if len(timestamps) == chunk_size:
        week_trading_intervals_map[week] = timestamps
    
    # Handle last week in year (less than 7 days of timestamps). Add remaining to mapping dictionary.
    else:
        week_trading_intervals_map[week-1] = week_trading_intervals_map[week-1].append(timestamps)
...
# Container for DataFrames which will contain reduced set of operating for each week
weekly_scenarios = list()

# For each week find reduced set of operating scenarios and their respective durations
for week_index, timestamps in week_trading_intervals_map.items():
    print('Processing week: {}'.format(week_index))
    
    # Get clustered scenarios
    df_clustered, _ = create_scenarios(df=df_o[timestamps], k=10, max_iterations=100, stopping_tolerance=10)
    
    # Update column index
    df_clustered.columns = pd.MultiIndex.from_product([[week_index], df_clustered.columns], names=['week', 'scenario'])
    
    # Append to list containing scenario data for each week of 2017
    weekly_scenarios.append(df_clustered)
    
# Concatenate DataFrames containing reduced set of weekly scenarios
df_weekly_scenarios = pd.concat(weekly_scenarios, axis=1)
...
with open(os.path.join(output_dir, 'weekly_scenarios.pickle'), 'wb') as f:
    pickle.dump(df_weekly_scenarios, f)
