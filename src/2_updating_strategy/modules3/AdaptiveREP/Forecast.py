...
class ConstructForecast:
    ...
    def get_forecast(self, forecast_type, forecast_intervals, forecast_uncertainty_increment, shock_option, initial_permit_price, model_horizon):
        ...
        if forecast_type == 'generator_energy':
            ...
            for calibration_interval in range(1, model_horizon + 1 - forecast_intervals + 1):
                forecast[calibration_interval] = dict()

                # For each forecast interval (lookin)
                for forecast_interval in range(1, forecast_intervals + 1):
                    ...
                    for generator, output in benchmark_week_metrics['dispatchable_generator_energy_MWh'][calibration_interval + forecast_interval - 1].items():
                        forecast[calibration_interval][forecast_interval][generator] = np.random.uniform(1 - (forecast_uncertainty_increment * forecast_interval + 1), 1 + (forecast_uncertainty_increment * forecast_interval)) * output
